...
/***
 * ((void *)0x00100100) and ((void *)0x00200200) are specific memory addresses chosen as poison values.
 *  These addresses are not intended to point to valid memory in typical scenarios, so if a poisoned pointer is mistakenly used, 
 * it will likely cause a crash or trigger a debugging tool, making it easier to track down the bug.
*/

/**
 * POISON_POINTER_DELTA is an offset applied to the base poison value. 
 * In your case, it’s defined as 0, meaning no additional offset is applied, so the actual poison values are simply 0x00100100 and 0x00200200.
 *In some systems, POISON_POINTER_DELTA might be set to a non-zero value to further randomize or adjust the poison addresses, 
 *depending on system architecture or security measures.
*/
